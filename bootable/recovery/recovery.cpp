...
//Lenovo Start 998:[miaotao1]Adding LenovoOTA
...
#endif //SUPPORT_DATA_BACKUP_RESTORE
  { "factory_wipe_data", no_argument, NULL, 'o' },
  { "copy_lenovo_resource", no_argument, NULL, 'a' },
  { NULL, 0, NULL, 0 },
};
...
static const char *SDCARD_EXT_ROOT = "/sdcard2"; /* Bug1041, added by lenovo-sw liujg6, 20130108 */
#if defined(MTK_SHARED_SDCARD)
static const char *FUSE_SDCARD_ROOT = "/data/media/0"; //factory reset for fuse, 20130522, added by xuchao2
#endif
...
static const char *UPDATE_TEMP_FILE = "/data/update.zip"; /* Bug1041, added by lenovo-sw liujg6, 20130108 */
#if 1 //wschen 2012-07-10
...
#endif
static const char *LENOVO_RESOURCE_FILE = "/sdcard2/resource.zip";
static const char *LENOVO_SDCARD_RESOURCE = "/sdcard/resource.zip";
static const char *LENOVO_SDCARD_EXT_RESOURCE = "/sdcard2/resource.zip";

static const char * EASYIMAGE_IN_DATA = "/data/easyimage.zip";
#if !defined(MTK_SHARED_SDCARD)
    static const char *LENOVO_RESOURCE_PATH = "/sdcard";
#else
    static const char *LENOVO_RESOURCE_PATH = "/data/media/0";
#endif
RecoveryUI* ui = NULL;
char* locale = NULL;
...
/* begin of Bug1041, added by lenovo-sw liujg6, 20130108 */
...
#endif
static bool LenovoExtractResource(const char* path)
{
	int err;
    if (ensure_path_mounted(path) != 0) {
        return false;
    ...
    if (ensure_path_mounted(LENOVO_RESOURCE_PATH) != 0) {
        return false;
    }
    ...
    bool success = mzExtractRecursive(&zip, "lenovo/", LENOVO_RESOURCE_PATH,
    ...
    if (ensure_path_mounted(path) != 0) { 
        ui->Print("Sdcard non-exist!!\n");
        return false;
    }  

	//because mzExtractRecursive do not know the sdcard_ext unplug. so double check.
	err = mzOpenZipArchive(path, &zip);
    if (err != 0) {
        ui->Print("Sdcard non-exist!!\n");
		return false;
    ...
    ui->Print("Unzip %d !\n", success);
	//unlink(LENOVO_SDCARD_RESOURCE);
	
	return success;
}

static bool cp_lenovo_resource(const char* original_path)
{
    ...
    if (ensure_path_mounted(original_path) != 0) {
        LOGE("Can't mount %s\n", original_path);
		ui->Print("External Sdcard Failed!\n");
        return false;
    }

    if (ensure_path_mounted(LENOVO_SDCARD_RESOURCE) != 0) {
        LOGE("Can't mount %s\n", LENOVO_SDCARD_RESOURCE);
        return false;
    }

    if ((fi = fopen(original_path, "rb")) == NULL)  {
        LOGE("Can't open %s\n", original_path);
		ui->Print("Lenovo Resource non-Exist!\n");
        return false;
    }
    ...
    if ((fo = fopen(LENOVO_SDCARD_RESOURCE, "wb")) == NULL)  {
        LOGE("Can't create %s\n", LENOVO_SDCARD_RESOURCE);
        goto EXIT;
    }
	...
	while ((read = fread(buf, 1, 4096, fi)) > 0) { 
		if (fwrite(buf, 1, read, fo) != read) {
			ui->Print("Copy Faile!\n");
    		goto EXIT;
		}
		done += read;
...
char cmd[512] = {0};
char white_list[128][256] = {{0}};
static int
lenovo_chmod_resource() {
    ensure_path_mounted("/system/");
    FILE *whitelist = fopen("/system/etc/whitelist", "r");
	int j, w_size = 0;
    char name[288];

    if(whitelist != NULL)
    {
        fseek(whitelist, 0, SEEK_SET);
        while(fgets(name, 256, whitelist))
        {
            ui->Print("lenovo_chmod_resource %s\n", name);
            memcpy(&white_list[w_size][0], name, strlen(name));
            if(white_list[w_size][strlen(name)-1] == 10)// ""
                white_list[w_size][strlen(name)-1] = 0;
            w_size++;
            memset(name, 0, 288);
            if(w_size >= 128) break;
        }
    ...
    {
        memset(name, 0, 288);
        sprintf(name, "/data/media/0/%s", &white_list[j][0]);
        ui->Print("Chmod %s\n", name);;
        dirSetHierarchyPermissions(name, 1023, 1023, 0775, 0666);
    }
...
static int
lenovo_clear_directory(const char* path) {
    ensure_path_mounted(path);

	//1. traversal the sdcard, save all the dir and file.
    DIR* d;
    struct dirent* de;
    d = opendir(path);
    if (d == NULL) {
	...
	//2. read the whitelist
    int i,j;
	int w_size = 0;
	
    ensure_path_mounted("/system/");
    //ensure_path_mounted("/fac/");
    ...
    for (i = 0; i < w_size; ++i)
        ui->Print("whitelist %s\n", &white_list[i][0]);

    //3. del all the unused file and dir.
    for (i = 0; i < z_size; ++i) 
    {
        int is_whitelist = 0;

        if(strlen(zips[i]) >= 255){
            ui->Print("File error %d !! %s\n", i, zips[i]);
            ensure_path_unmounted("/system/");
            //ensure_path_unmounted("/fac/");
            return -1;
		}

        for(j = 0; j < w_size; ++j)
        {
            if(!strcmp(zips[i], &white_list[j][0]))
            {
                is_whitelist = 1;
                ui->Print("Reserved %s\n", &white_list[j][0]);
                white_list[j][0] = 0;
        ...
        {
            memset(cmd, 0, 512);
            //sprintf(cmd, "/system/bin/rm -r /sdcard/%s", zips[i]);
            sprintf(cmd, "/system/bin/rm -r \"%s/%s\"", path, zips[i]);
			//ui->Print("clarence - %s\n", cmd);
            system(cmd);
        }
    }
...
#if defined(MTK_SHARED_SDCARD)
//wipe data except fuse sdcard, 20130522, added by xuchao2
static int
lenovo_clear_directory_keep_fuse(const char* path, const char* path_white_list) {
    ensure_path_mounted(path);
    ...
    FILE *whitelist = fopen(path_white_list, "r");
    if(whitelist != NULL)
    {
        char name[256];
        fseek(whitelist, 0, SEEK_SET);
    ...
    for (i = 0; i < z_size; ++i) 
    {
        ...
        if(!is_whitelist)
        {
            ...
            sprintf(cmd, "/system/bin/rm -r \"%s/%s\"", path, zips[i]);
			//ui->Print("clarence - %s\n", cmd);
            if(strcmp(".layout_version",zips[i]))
			system(cmd);
        }
    }
...
//add by mengfd1,for delete whitelist
static int
lenovo_clear_whistlist(const char* path) {    
    DIR* d;
    int w_size = 0;
    ensure_path_mounted(path);
    d = opendir(path);
    ...
    if(whitelist != NULL)
    {
        ...
        while(fgets(name, 256, whitelist))
        {
            ...
            memset(cmd, 0, 512);
            sprintf(cmd, "/system/bin/rm -r \"%s/%s\"", path, name);
            system(cmd);
            w_size++;
            if(w_size >= 128) break;
        } 
...
/* begin of Bug1041, added by lenovo-sw liujg6, 20130108 */
static void write_result_file_lenovo(const char *file_name, int result)
{
    char  dir_name[256];
    ...
    *p = 0;
    ...
    // modify by humina 2012-04-25 begin
   // char buf[4];
  ...
  //   write(result_fd, buf, 1);
    ...
    // modify by humina 2012-04-25 end
...
main(int argc, char **argv) {
    ...
    while ((arg = getopt_long(argc, argv, "", OPTIONS, NULL)) != -1) {
        ...
        case 'o': factory_wipe_data = 1; break;
        case 'a': copy_lenovo_resource = 1; break;
    ...
    if (update_package != NULL) {
        ...
        lenovo_ota_remove_package_and_write_result(update_package,status);
        //@}
        if (status == INSTALL_SUCCESS && wipe_cache) {
            if (erase_volume("/cache")) {
...
/* begin of Bug1041, added by lenovo-sw liujg6, 20130108 */
        ...
        } /* Bug1041, added by lenovo-sw liujg6, 20130108 */
...
/* end of Bug1041 */
        ...
        if (device->WipeData()) status = INSTALL_ERROR;
	   #if defined(MTK_SHARED_SDCARD)
	   //factory reset for fuse, 20130522, added by xuchao2
       ensure_path_mounted("/data");
      lenovo_clear_directory_keep_fuse("/data","/system/etc/whitelist_media");
      lenovo_clear_directory_keep_fuse("/data/media","/system/etc/whitelist_media_0");
	  lenovo_clear_directory(FUSE_SDCARD_ROOT);
	  #else
	  if (erase_volume("/data")) status = INSTALL_ERROR;
	  #endif
	  if (wipe_cache && erase_volume("/cache")) status = INSTALL_ERROR;
      if (status != INSTALL_SUCCESS) ui->Print("Data wipe failed.\n");
      #if !defined(MTK_SHARED_SDCARD)
          lenovo_clear_directory(SDCARD_ROOT);
      #endif
        }else if(copy_lenovo_resource) {
#if 1
        struct bootloader_message boot;
        memset(&boot, 0, sizeof(boot));
        strlcpy(boot.command, "boot-recovery", sizeof(boot.command));
        ...
        if(1)
        {
            ...
            int copyfile = 0;
            int unzipfile = 0;
            //clarence-modify copy resource, using product name
            char product[PROPERTY_VALUE_MAX] = {0};
            char resourse_path[PROPERTY_VALUE_MAX+18] = {0};
            property_get("ro.build.product", product, ""); 
#if defined(MTK_SHARED_SDCARD)
            sprintf(resourse_path, "/sdcard/%s.zip", product);
#else
            sprintf(resourse_path, "/sdcard2/%s.zip", product);
#endif
            ui->ShowText(true);
            ui->Print("Copy Lenovo Resource Begin - %s.\n", resourse_path);

            for(i=0; i<3; i++)
            {
                if(!copyfile)
                {
                    //if(cp_lenovo_resource(LENOVO_RESOURCE_FILE))
                    {
                        copyfile = 1;
                    }
                }
                if(copyfile && !unzipfile)
                {
                    if(LenovoExtractResource(resourse_path/*LENOVO_SDCARD_EXT_RESOURCE*/))
                    {
                        unzipfile = 1;
                    }
                }
            ...
            if (status != INSTALL_SUCCESS)
            {
                ui->Print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
                ui->Print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
                ui->Print("--Copy Lenovo Resource Failed.--\n");
                ui->Print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
                ui->Print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
            }
            else
            {
                ...
                lenovo_chmod_resource();
#endif
                ui->Print("Copy Lenovo Resource OK.\n");
            }

            ui->Print("Copy Lenovo Resource End.\n");
        }
        /* end of Bug1041 */
    } else if (wipe_cache) {
#if 1 //wschen 2012-03-21
